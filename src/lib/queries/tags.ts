  ...
  return cached(`getAllTagsWithCount:${lang}`, () =>
    sanityClient.fetch<TagWithCount[]>(
      `*[_type == "tag" && language == $lang]{
        ...
        "postCount": count(*[_type == "post" && language == $lang && references(^._id) && defined(slug.current) && count(body) > 0])
...
export function getTagPosts(tagSlug: string, lang = DEFAULT_LOCALE) {
  return cached(`getTagPosts:${tagSlug}:${lang}`, () =>
    sanityClient.fetch<PostCard[]>(
      `*[_type == "post" && language == $lang && defined(slug.current) && count(body) > 0 && references(*[_type == "tag" && slug.current == $tagSlug]._id)] | order(coalesce(createdAt, _createdAt) desc) ${POST_CARD}`,
      { tagSlug, lang },
    ),
  );
}
  ...
  return cached(`getTagsWithPostPreview:${lang}:${minPosts}`, () =>
    sanityClient.fetch<TagWithPosts[]>(
      `*[_type == "tag" && language == $lang]{
        _id, title, "slug": slug.current,
        "postCount": count(*[_type == "post" && language == $lang && references(^._id) && defined(slug.current) && count(body) > 0]),
        "posts": *[_type == "post" && language == $lang && references(^._id) && defined(slug.current) && count(body) > 0] | order(coalesce(createdAt, _createdAt) desc) [0...4] ${POST_CARD}
